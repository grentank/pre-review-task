...
describe('TerrainHelper', () => {
  ...
  describe('score difficulty / оценка сложности', () => {
    ...
    it("raises an exception if it can't recognize the terrain / выдает ошибку, если не может распознать местность", () => {
      expect(() => {
        helper.scoreDifficulty('clouds');
      }).toThrow(
        new Error(
          `I can't recognize this terrain type: clouds (must be water, plains, forest, lava, or mountains)`,
        ),
      );
    });
  });
  ...
  describe('convering letters to terrain / преобразует буквы в местности', () => {
    ...
    it("raises an exception if it can't recognize the terrain / выдает ошибку, если не может распознать местность", () => {
      expect(() => {
        helper.letterToTerrain('C');
      }).toThrow(
        new Error("I can't recognize this terrain letter: C (must be 'W', 'P', 'F', 'L' or 'M')"),
      );
    });
  });
});
